...
describe('Protractor Security RAT editor and export testsuites', function () {
    ...
    it('Test requirements generation and change settings', function () {
        ...
        expect(element.all(by.className('disabledButton')).count()).toBeGreaterThan(1);
        browser.sleep(5000);
    ...
    it('Export to ticket and create a ticket without being authenticated', function () {
        ...
        switchToTab0();
        browser.sleep(1000);
        element.all(by.buttonText(browser.params.statusColumnDefaultValue)).first().click();
        (element(by.linkText(browser.params.otherStatusColumnValues[0]))).click();
        (element(by.buttonText(SaveButton))).click();
        //		browser.sleep(3000);
