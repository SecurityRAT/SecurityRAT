...
describe('Protractor Security RAT implementation type entity testsuite', function() {
	...
	it('adminisetring an implementation type', function() {
		...
		.then(function(instanceArray) {
            ...
            element.all(by.options("statusColumn as statusColumn.name for statusColumn in statuscolumns track by statusColumn.id")).get(1).click();
            element.all(by.options("optColumn as optColumn.name for optColumn in optcolumns track by optColumn.id")).get(1).click();
