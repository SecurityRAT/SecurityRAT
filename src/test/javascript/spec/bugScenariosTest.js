...
describe('Protractor Security RAT bug Scenarios Testsuite', function () {
    ...
    it('Test bug where update available in not working', function () {
            ...
            }); element(by.partialButtonText('Generate')).click(); browser.sleep(3000); browser.wait(function () {
            return element(by.buttonText('Close')).isPresent();
        }); element(by.buttonText('Close')).click(); browser.sleep(1000); element(by.buttonText('Updates available')).isPresent().then(function () {
            element(by.buttonText('Updates available')).click();
            browser.wait(function () {
                return element(by.buttonText('Close')).isPresent();
            });
        ...
        refreshBrowser(); browser.get(browser.params.impTestAttachmentUrl + browser.params.attachmentUrls[3]).then(function () {}, function () {
            browser.switchTo().alert().accept();
        }); browser.sleep(5000);
        (element(by.buttonText('Close'))).click(); browser.sleep(3000);
        ...
        element(by.buttonText('Updates available')).isPresent().then(function () {
            ...
            element(by.buttonText('Save')).click();
            rowser.get(browser.params.testHost);
            generateRequirementSet('test artifact', 'External');
            element(by.buttonText('Search')).click();
            element(by.model('search')).sendKeys('test broken view in editor');
            browser.sleep(2000);
        ...
        it('Test bug where update available in not working', function () {
            ...
            (element(by.buttonText('Close'))).click(); browser.sleep(3000);
            var requirementCounts = element(by.binding('requirements.length')).getText(); browser.sleep(3000); element(by.buttonText('Close')).click(); browser.sleep(3000); element(by.buttonText('Cancel the updates')).isPresent().then(function () {
                var removeReq = element.all(by.id('removeReq'));
                expect(removeReq.count()).toBeGreaterThan(0);
                expect(element.all(by.id('acceptReq')).count()).toBeGreaterThan(0);
