...
function write_cellXfs(cellXfs) {
	...
	var xfinnertags = null;
	cellXfs.forEach(function(c) { 
		// custom code from jay: begin
		if(c['xfinnertags']  && c['xfinnertags'].length > 0){
			xfinnertags = [];
			c['xfinnertags'].forEach(function(xfinnertag) {
				switch(xfinnertag.name) {
				case 'alignment': xfinnertags[xfinnertags.length] = writextag('alignment', null, xfinnertag.alignment);c.applyAlignment = 1;break;
				}
				xfinnertags = xfinnertags.join("");
			})
		}
		// custom code from jay: end
		o[o.length] = (writextag('xf', xfinnertags, c)); 
	});
	o[o.length] = ("</cellXfs>");
...
function write_sty_xml(wb, opts) {
	...
	// custom code from jay: begin
	o[o.length] = ('<fonts count="3"><font><sz val="12"/><color theme="1"/><name val="Calibri"/><family val="2"/><scheme val="minor"/></font><font><b/><sz val="13"/><color theme="1"/><name val="Calibri"/><family val="2"/><scheme val="minor"/></font><font><b/><sz val="20"/><color theme="1"/><name val="Calibri"/><family val="2"/><scheme val="minor"/></font></fonts>');
	// custom code from jay: end
...
function get_cell_style(styles, cell, opts) {
	...
	for(var i = 0, len = styles.length; i != len; ++i) 
		if(styles[i].numFmtId === z && styles[i].fontId === cell.fontId
		// custom code from jay: begin		
		&& cell.xfinnertags === undefined
		// custom code from jay: end
		) return i;
	styles[len] = {
		numFmtId:z,
		fontId:cell.fontId ? cell.fontId: 0,
		fillId:0,
		borderId:0,
		xfId:0,
		applyNumberFormat:1,
		// custom code from jay: begin
		xfinnertags: cell.xfinnertags ? cell.xfinnertags : undefined
		// custom code from jay: end				
	};
	return len;
}
...
function write_ws_xml_data(ws, opts, idx, wb) {
	...
	for(R = range.s.r; R <= range.e.r; ++R) {
		...
		// custom code from jay: begin
		if(r.length > 0) o.o[o.o.length] = (writextag('row', r.join(""), {r:rr}));
		// custom code from jay: end
...
//		rdata = write_ws_xml_data(ws, opts, idx, wb);
	...
	if(idx === 0 && dropdownListIndexes.length > 0) {
		...
		dropdownListIndexes.forEach(function(element) {
			str += '<x14:dataValidation type="list" allowBlank="1" showInputMessage="1" showErrorMessage="1"><x14:formula1><xm:f>';
			str += wb.SheetNames[idx+1] + "!$";
			str += element.valuesIndexes.start.col + '$' + element.valuesIndexes.start.row;
			str += ':$' + element.valuesIndexes.end.col + "$" + element.valuesIndexes.end.row;
			str += "</xm:f></x14:formula1><xm:sqref>";
			str += element.dropdownStartIndex.col + element.dropdownStartIndex.row;
			str += ':';
			str += element.dropdownEndIndex.col + element.dropdownEndIndex.row;
			str += '</xm:sqref></x14:dataValidation>'
			
		})
		str += '</x14:dataValidations></ext></extLst>'
		o[o.length] = str;
	}
	// custom code from jay:end
