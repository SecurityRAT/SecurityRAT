...
angular.module('sdlctoolApp')
  .controller('customRequirementController', function ($scope, $location, $uibModalInstance, sharedProperties, $filter, appConfig, $interpolate) {
	  $scope.crObject = sharedProperties.getProperty();
	  $scope.requirement = {};
	  ...
	  $scope.init = function() {
		  if(angular.isUndefined($scope.crObject.requirements)) {
			  ...
			  $scope.req = $scope.crObject.requirements[0];
...
//		  $scope.updateOptColumns();
		  if($scope.status.add) {
			  ...
			  item = {requirement: $scope.requirement, categoryIndex: categoryIndex}
